...
async function authenticate(username, password) {
    ...
    let dbUsers = await Db.query('SELECT * FROM user WHERE username = ?', [username]);
...
async function setUserPassword(username, password) {
    await Db.query('UPDATE user SET password = :pwd WHERE username = :username', {
...
function requireLogin(req, res, next) {
  if (!req.session.user) {
    res.flash('error', 'Pre prístup sa musíte prihlásiť.');
    return res.redirect('/user/login');
  }
  next();
}

export {authorize, authenticate, setUserPassword, requireLogin, hashPassword}